...
const Card: React.FC<CardProps> = ({ items }) => {
  return (
    <div className="min-h-screen px-4 md:px-20 lg:px-40 pt-[11rem] pb-10 bg-gray-50">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {items.length === 0 ? (
          ...
          items.map((item) => (
            ...
            >
                {item.imageUrl&&(
                    <img src={item.imageUrl||'https://via.placeholder.com/150'} alt={item.title}  className='w-full h-48 object-cover'/>
                )}
...
<div className='absolute flex justify-center items-center p-2 bg-white rounded-full top-3 rigth-3 cursor-pointer'>
